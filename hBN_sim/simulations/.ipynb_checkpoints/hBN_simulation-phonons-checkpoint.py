...
data_dir = os.path.abspath(os.path.join(parent_dir, 'data', 'data_phonons'))#choose path for results
...
#insert array of wanted number of phonon configurations
...
if __name__ == "__main__":
    ...
    try: 
        ...
        for num_configs in num_configs_list:
            ...
            f'_filledwith{defect_atom_symbol}_energy{int(energy_probe)}_numconfigs{num_configs}'\
            ...
            variables_list = [timestamp, str(size_x) + 'x' + str(size_y), defect_type, vacancy_atom,
                              defect_atom_symbol, int(energy_probe), num_configs, semiangle_cutoff,
