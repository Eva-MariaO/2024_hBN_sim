...
def simulation(symbol, aberration_coefficients, astig_angle, path, variables_list):
    ...
    measurements_total.to_zarr(f'./data/data_aberrations/{path}_RAW.zarr')
    ...
    #run simulation for all variations
        for symbol in defect_atom_symbol_list:
            for ab in aberration_coefficients_list:
                ...
                path = (f'{timestamp}_hBN_size{size_x}x{size_y}_{defect_type}_vacancy{vacancy_atom}_filledwith{symbol}_energy{int(energy_probe)}_defocus{defocus}_Cs{int(Cs)}_astig{astig}_astigangle{astig_angle}_coma{coma}')#_focal{int(focal_spread)}_ang{int(angular_spread)}')
                variables_list = [timestamp, str(size_x) + 'x' + str(size_y), defect_type, vacancy_atom, symbol,
