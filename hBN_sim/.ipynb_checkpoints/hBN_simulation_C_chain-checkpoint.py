...
defect_type = 'ring' #'ring'] #'single'/'double'/'ring' defects
...
def simulation(len_C_chain, path, variables_list):
    ...
    #determine lenght of C_chain (up to full ring)
    ...
    #export data
    measurements_total.to_zarr(f'./data/data_C_chain/{path}_RAW.zarr')
...
if __name__ == "__main__":
    ...
    #run simulation for all variations
        for len_C_chain in len_C_chain_list:
            ...
            path = (f'{timestamp}_hBN_size{size_x}x{size_y}_{defect_type}_vacancy{vacancy_atom}_filledwith{defect_atom_symbol}_energy{int(energy_probe)}_len_C_chain{len_C_chain}_Cs{int(Cs)}_astig{astig}_astigangle{astig_angle}_coma{coma}')#_focal{int(focal_spread)}_ang{int(angular_spread)}')
            variables_list = [timestamp, str(size_x) + 'x' + str(size_y), defect_type, vacancy_atom, defect_atom_symbol,
         int(energy_probe), len_C_chain, semiangle_cutoff, int(Cs), astig, astig_angle, coma]
            print(path)
            count += 1
